...
function drawProductsUIInCart(allProducts = []) {
  ...
  if (JSON.parse(localStorage.favoriteProducts).length === 0) {
  ...
  let products = JSON.parse(localStorage.favoriteProducts || allProducts);
  let productUI = products.map((item) => {
    ...
    <div class="product_item_action">
       <button class="remove_from_cart btn" onclick = 'removeFromFav(${item.id})'>
          Remove From Favorite Cart<i class="fas fa-times"></i>
       </button>
    </div>
    </div>`;
  });
...
function removeFromFav(id) {
  if (localStorage.favoriteProducts) {
    ...
    localStorage.favoriteProducts = JSON.stringify(filterItems);
